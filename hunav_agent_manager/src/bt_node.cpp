...
namespace hunav
{
  ...
  void BTnode::initializeBehaviorTree(hunav_msgs::msg::Agents agents)
  {
    ...
    for (unsigned int i = 0; i < agents.agents.size(); i++)
    {
      ...
      case hunav_msgs::msg::AgentBehavior::BEH_SURPRISED: // ############################### TEMPORARY TESTBT  ###############################
        ...
        trees_[agents.agents[i].id] =
            ...
            // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_walk_together_2.xml", blackboard);
            // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_say_something.xml", blackboard);
        break;

      case hunav_msgs::msg::AgentBehavior::BEH_SCARED: // ############################### TEMPORARY TESTBT  ###############################
        blackboard->set<double>("duration", agents.agents[i].behavior.duration);
        blackboard->set<bool>("once", agents.agents[i].behavior.once);
        blackboard->set<double>("dist", agents.agents[i].behavior.dist);
        ...
        try
        {
          trees_[(int)agents.agents[i].id] =
              ...
              // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_walk_together.xml", blackboard);
              // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_conversation_circle.xml", blackboard);
              // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_conversation_circle_nearest.xml", blackboard);
              // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_go_to_2.xml", blackboard);
              // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_say_something.xml", blackboard);
              // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_approach.xml", blackboard);
              // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_is_anyone_speaking.xml", blackboard);
              // factory_.createTreeFromFile(pkg_shared_tree_dir_ + "TestBT_is_anyone_looking_at_me.xml", blackboard);
